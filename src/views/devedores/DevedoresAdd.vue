    ...
    <div>
        ...
        <div class="container">
            ...
            <form class="row">
                ...
                <div class="col-md-12">
                    <button type="button" class="btn btn-primary" @click="saveDevedor">Cadastrar</button>
                </div>
            </form>
        </div>
    </div>
...
export default {
    ...
    methods: {
        saveDevedor() {
            ...
            axios.post(url('/devedores'), JSON.stringify(data), headers)
            .then(response => {
                if(response.data.status == 'success') {
                    alert(response.data.message);
                    this.$router.push('/devedores');
                }
                else {
                    alert(response.data.message);
                }
